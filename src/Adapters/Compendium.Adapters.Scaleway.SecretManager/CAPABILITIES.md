...
- **Disabled-version access**: the provider's 4xx on access is disambiguated
  ...
  `VersionDisabled` vs `VersionNotFound` vs `SecretNotFound` code. It arms on
  an ambiguous 4xx only — a server status (5xx) or a transport fault always
  yields `SecretVault.ProviderRejected` carrying the real status in
  `statusCode` metadata, and costs no extra request. Already-precise failures
  (`AuthenticationFailed`, `Throttled`, `QuotaExceeded`) are not rewritten
  either. The same rule guards the enable/disable idempotency probe: a 5xx
  stays a failure, never a no-op success.
